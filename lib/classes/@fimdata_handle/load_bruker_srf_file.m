...
try
    ...
    if data_file>=3 % successfully opened
        ...
        for probe_id=1:t_size
            ...
            %[xgrid,ygrid,zgrid]=ndgrid(X_scale,Y_scale,linspace(min(Z_pos),max(Z_pos),X_size));
            %[a,b,c]=griddata(X_pos(currentprobeidx),Y_pos(currentprobeidx),Z_pos(currentprobeidx),val(valid(currentprobeidx),20),xgrid,ygrid,zgrid);
            for zslice=1:Z_size
                ...
                %temp=hist3([X_pos(currentzidx),Y_pos(currentzidx)],{X_scale,Y_scale});
