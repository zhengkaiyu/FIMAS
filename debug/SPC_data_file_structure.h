...
typedef struct {     // extension of SPCBinHdr
  ...
  unsigned long   extension[62];    //  for future use
  }SPCBinHdrExt;
...
typedef struct{        // structure for Z scan  Axio Observer.Z1
  short              connected;   
  unsigned short     dev_type;   // currently only 1 for Axio Observer.Z1
  double             start_pos;
  double             step_width;
  double             min_pos;
  double             max_pos;
...
typedef struct _MeasureInfo {
  ...
  short    dig_flags;     // SP_MST_CLK parameter - digital flags : 
  ...
  char     reserve[65];         // total size of MeasureInfo = 512 bytes
  }MeasureInfo;
