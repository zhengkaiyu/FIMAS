function [ result ] = op_FluxMap( data_handle, option, varargin )
    ...
    case 'add_data'
        ...
        switch data_handle.data(current_data).datatype
            case {'3D_xyT_image','3D_parameter_mapstack'}
                ...
                result.datatype='3D_vector_mapstack';
    ...
    case 'calculate_data'
        ...
        switch data_handle.data(current_data).datatype
            case {'3D_xyT_image','3D_parameter_mapstack','3D_vector_mapstack'}%originated from 3D/4D traces_image
                ...
                dcdT=diff(data,1,3);
                D=zeros(2,newxlim,newylim,newTlim);
                for T_idx=2:newTlim
                    [D(1,:,:,T_idx-1),D(2,:,:,T_idx-1)]=gradient(squeeze(dcdT(:,:,T_idx-1)./data(:,:,T_idx-1)),dx,dy);
                    %[D(1,:,:,T_idx-1),D(2,:,:,T_idx-1)]=gradient(squeeze(dcdT(:,:,T_idx-1)),dx,dy);
                    %D(:,:,:,T_idx-1)=squeeze(D(:,:,:,T_idx-1))./permute(repmat(data(:,:,T_idx-1),[1,1,2]),[3,1,2]);
                end
                %data=smooth3(squeeze(data),'box',[xbin,ybin,Tbin]);
                %D=zeros(2,px_lim,py_lim,pT_lim);
                ...
                %    [D(1,:,:,T_idx),D(2,:,:,T_idx)]=gradient(data(:,:,T_idx-1),dx,dy);
                    
                %end
                %D=permute(D,[4,1,2,3]);D(:,:,bg)=nan;D=permute(D,[2,3,4,1]);
                %dist=sqrt(D(1,:).^2+D(2,:).^2);
                %dist=reshape(dist,[1,newxlim,newylim,newTlim]);
                %D=bsxfun(@rdivide,D,dist);
