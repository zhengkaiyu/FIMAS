...
parameters=struct('note','',...
    ...
    'norm_type','poisson',... % noise correction type (none/poisson/variance)
...
try
                    ...
                    case 'norm_type'
                        switch val
                            case {'none','poisson','variance'}
                                data_handle.data(current_data).datainfo.norm_type=val;
                            otherwise
                                %default to poisson noise cancellation
                                data_handle.data(current_data).datainfo.norm_type='poisson';
                        end
                        status=true;
                    case 'bg_threshold'
                         val=str2double(val);
            ...
            for current_data=data_idx
                ...
                switch data_handle.data(parent_data).datatype
                    case {'DATA_IMAGE'}%originated from 3D/4D traces_image
                        ...
                        fval=calculate_NCPCA(fval,data_handle.data(current_data).datainfo.norm_type,...
    ...
    function val=calculate_NCPCA(data,NormType,threshold,MaxDims)
        datasize=size(data);
        ...
        data=bsxfun(@times,(data-round(Bg)),mask_ncpca);%Bg is single value
        ...
        %allvecs=transpose(double(reshape(mydata_norm,[size(mydata_norm,1)*size(mydata_norm,2) size(mydata_norm,3)])));
        
        %Data is centered
        data_norm_cent=bsxfun(@minus, data_norm,mean(data_norm,2));
        
        %covariance matrix of dataset
        myc = (data_norm_cent*data_norm_cent')/(size(data_norm_cent,2)-1);
        
        %SVD of covariance matrix
        [U D V] = svd(transpose(myc));
    ...
    %{
        ...
        if prod(size(NoiseCorrection)) > 1
